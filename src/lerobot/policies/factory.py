...
def make_policy(
    ...
    if ds_meta is not None:
        # For ACT policy, exclude observation.environment_state as it's for debugging only
        # and the modified ACT architecture doesn't use it
        exclude_features = []
        if cfg.type == "act":
            exclude_features = ["observation.environment_state"]
        features = dataset_to_policy_features(ds_meta.features, exclude_features=exclude_features)
    else:
